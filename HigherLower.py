from Art import logo_higher_lower, vs
from game_data import higher_lower_data
import random
from replit import clear



def format_data(account):
    """Takes the account data and returns the printable format."""
    account_name = account["name"]
    account_descr = account["description"]
    account_country = account["country"]
    return f"{account_name}, a {account_descr}, from {account_country}"


def check_answer(guess, a_followers, b_followers):
    """Takes the user guess and follower counts and returns if they got it right."""
    if a_followers > b_followers:
        return guess == "a"
    else:
        return guess == "b"     


print(logo_higher_lower)
score = 0
game_should_continue = True
account_b = random.choice(higher_lower_data)

while game_should_continue:

    account_a = account_b
    account_b = random.choice(higher_lower_data)

    if account_a == account_b:
        account_b = random.choice(higher_lower_data)


    print(f"Compare A: {format_data(account_a)}.")
    print(vs)
    print(f"Against B: {format_data(account_b)}.")

    guess = input("Who has more followers? Type 'A' or 'B': ").lower()

    a_follower_count = account_a["follower_count"]
    b_follower_count = account_b["follower_count"]
    is_correct = check_answer(guess, a_follower_count, b_follower_count)

    clear()
    print(logo_higher_lower)
    
    if is_correct:
        score += 1
        print(f"You're right! Current score: {score}")
    else:
        game_should_continue = False
        print(f"Sorry, that's wrong. Final score: {score}")



# game = False

# print(logo_higher_lower)

# while not game:
#     score = 0
#     random_number1 = randint(0,49)

#     name1 = higher_lower_data[random_number1]["name"]
#     follower1 = int(higher_lower_data[random_number1]["follower_count"])
#     description1 = higher_lower_data[random_number1]["description"]
#     country1 = higher_lower_data[random_number1]["country"]

#     print(f"Compare A: {name1}, a {description1}, from {country1}.")

#     print(vs)

#     random_number2 = randint(0,49)

#     name2 = higher_lower_data[random_number2]["name"]
#     follower2 = int(higher_lower_data[random_number2]["follower_count"])
#     description2 = higher_lower_data[random_number2]["description"]
#     country2 = higher_lower_data[random_number2]["country"]

#     print(f"Against B: {name2}, a {description2}, from {country2}.")

#     chose = input("Who has more followers? Type 'A' or 'B': ").lower()


#     if follower1 > follower2:
#         if chose == "a":
#             score += 1
#             print(f"You're right! Current score: {score}")
#         else:
#             game == True
#             print(f"Sorry, that's wrong. Final score: {score}")  

#     elif follower1 < follower2:
#         if chose == "b":
#             score += 1
#             print(f"You're right! Current score: {score}")
#         else:
#             game == True
#             print(f"Sorry, that's wrong. Final score: {score}")


